...
// NULL
static bool mayPointToNull(
    const std::string& var, PointsToInfo& PointsTo, int depth = 3) {
  if (depth <= 0)
    return false;
  ...
  for (const auto& t : pts) {
    ...
    if (it2 != PointsTo.end()) {
      if (mayPointToNull(t, PointsTo, depth - 1))
        return true;
    }
  }
  return false;
...
Domain::NullState PointerAnalysis::computeNullState(
    const std::string& var, PointsToInfo& PointsTo) {
  auto it = PointsTo.find(var);
  if (it == PointsTo.end())
  ...
  if (hasNull && !hasAddr)
    return dataflow::Domain::Null;
  if (!hasNull && hasAddr)
    return dataflow::Domain::NotNull;
  if (hasNull && hasAddr)
    return dataflow::Domain::MaybeNull;
  return dataflow::Domain::Unknown;
}
...
PointerAnalysis::PointerAnalysis(Function& F) {
  ...
  for (inst_iterator Iter = inst_begin(F), E = inst_end(F); Iter != E; ++Iter) {
    ...
    if (StoreInst* Store = dyn_cast<StoreInst>(Inst)) {
      ...
      if (it != NullStates.end() &&
          (it->second == Domain::Null || it->second == Domain::MaybeNull)) {
        errs() << "Possible null dereference (store) in " << FuncName << " at: " << *Store
               << "\n";
      }
    } else if (LoadInst* Load = dyn_cast<LoadInst>(Inst)) {
      ...
      if (it != NullStates.end() &&
          (it->second == Domain::Null || it->second == Domain::MaybeNull)) {
        errs() << "Possible null dereference (load) in " << FuncName << " at: " << *Load
               << "\n";
      }
      ...
      if (it != NullStates.end() &&
          (it->second == Domain::Null || it->second == Domain::MaybeNull)) {
        errs() << "Possible null dereference (getelementptr) in " << FuncName
               << " at: " << *GEP << "\n";
      }
    }
  }
...
extern "C" LLVM_ATTRIBUTE_WEAK llvm::PassPluginLibraryInfo llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, PASS_NAME, "1.0.0", [](llvm::PassBuilder& PB) {
